...
use geoverse::{DequeStorage, GeoCache, GeoCacheConfigBuilder, LirsStorage, LruStorage};
...
fn create_example_lirs_geo_cache() -> GeoCache<'static, LirsStorage> {
  let config = Box::leak(Box::new(GeoCacheConfigBuilder::default().build()));
  GeoCache::with_capacity(config, CAPACITY_SIZE)
}
...
fn bench_lirs_insert(c: &mut Criterion) {
  let mut group = c.benchmark_group("lirs_insert");
  ...
  for size in [100, 1_000, 10_000] {
    group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, &size| {
      b.iter_batched(
        ...
        |data| {
          let mut geo_cache = create_example_lirs_geo_cache();
          for (lat, addr) in data {
            geo_cache
              .insert(std::hint::black_box((lat, 17.1847104, "sk")), addr)
              .unwrap();
...
fn bench_lirs_get(c: &mut Criterion) {
  let mut group = c.benchmark_group("lirs_get");
